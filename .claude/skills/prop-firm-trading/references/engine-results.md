...
|---|---|---|---|---|
| [`engine-results/2026-09-22-full-sweep.md`](engine-results/2026-09-22-full-sweep.md) | 2026-09-22 | `4e398aa` | best firms and plans for his rules; per-plan eval ladders; funded sizing; sensitivities (contract caps, intraday path-walk, commission, payout size, live-trigger caps, idle days, win rate); DP; full doc-tree read; adversarial verification | its monthly-net numbers are stale: the printed `expectedMonthlyNet` metric changed after this run (horizon credit added, `--rebuy-lag-days` introduced), so re-measure on the current engine commit before citing any monthly-net figure from this run. Everything else is current as of 2026-09-22 |
| [`engine-results/2026-09-23-average-reward-dp-validation.md`](engine-results/2026-09-23-average-reward-dp-validation.md) | 2026-09-23 | uncommitted working tree on `65e310c` (staged diff sha256 `afcf8c61...`) | does the average-reward `optimize dp` hold up against the best flat funded policy on monthly net per slot (gate D11)? TopStep no-fee-standard and FTMO growth, eval-days 15, funded-days 252, rebuy-lag-days 0, seed 42, 20,000 trials | gate D11 FAILED: TopStep DP $251/mo vs best flat (flat $1000) $3,142/mo, 0.08x; FTMO DP stopped with an unconverged funded level. Superseded by `2026-09-23-average-reward-dp-validation-2.md` (the two defects it exposed are fixed) |
| [`engine-results/2026-09-23-average-reward-dp-validation-2.md`](engine-results/2026-09-23-average-reward-dp-validation-2.md) | 2026-09-23 | uncommitted working tree on `65e310c`, after the convergence and qualifying-day DP fixes | same gate D11 re-run after fixing the funded DP's per-level sweep cap and its missing qualifying-day payout gate; FTMO growth and TopStep no-fee-standard at the same inputs | gate D11 PASSED: FTMO DP $4,350/mo vs best flat (flat $800) $2,989/mo, 1.46x; TopStep DP $3,101/mo vs best flat (flat $1000) $3,142/mo, 0.99x. Current DP measurement; DP-predicted values still run about $2,400/mo optimistic |
...
|---|---|---|---|
| **monthly net** | `expectedMonthlyNet` = `(expectedNet + expectedHorizonCredit) x 21 / (expectedDaysPerTrial + rebuyLagDays x expectedAttempts)` (`simulator/engine.ts`); printed by `optimize funded` (`--sort monthly`, the default) and `optimize dp` | steady-state net cash per month for ONE account slot, refilled after every failed eval, funded bust or horizon end; an account still alive at the horizon is credited its withdrawable balance (`expectedHorizonCredit`, one payout request net of split, scheduling/qualifying-day gates ignored); `--rebuy-lag-days` (default 0) charges empty slot time per new eval attempt | **Yes.** Caveats: assumes instant, unlimited repurchase past `--rebuy-lag-days` (no purchase caps); the horizon credit models the withdrawable balance, not the days a real payout request takes to arrive (no payout lag modeled) |
| lifetime net (removed from the engine) | `lifetimeExpectedNet` = `(expectedNet - pBust x feesUntilPass) / (1 - pBust)` (formerly `core/LifetimeExtraction.ts`, deleted) | "repeat until one funded account survives the horizon" | **Removed, 2026-09-23.** No time cost, so it diverged as pBust approached 1 (Tradeify Lightning flat $300 scored $1.53M); it renewed only after a funded bust, never after a failed eval; the `- pBust x fee` term charged each renewal's fee twice, because each cycle's `expectedNet` already contained it. SKILL.md Hard Rule 8 named lifetime net as the funded objective; it was rewritten to name monthly net (replacement priced in) instead of being reconciled with this metric |

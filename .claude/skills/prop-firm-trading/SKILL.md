...
- **The real commands, current as of this writing:** `cli prop plans` (inspect a plan's validated rules — check `consistency eval`/`funded` here before reaching for the DP, see below), `cli prop compare` (rank many plans on one fixed policy), `cli prop ladder` (grid-search the eval-phase rung ladder for one plan, `--firm`/`--variant`), `cli prop sim` (one plan, one policy, full detail), `cli prop optimize funded` (sweep flat-$, percent-of-cushion, and — with `--funded-ladder`, comma-separated — a funded-phase ladder too, for one plan; `--sort lifetime` for a renewal-adjusted "repeat this forever" answer, `--sort cycle` for "how much do I expect from ONE bounded run" — **pick the sort key that matches the actual question before comparing candidates, and never compare a `lifetime`-sorted table's top row against a `cycle`-sorted table's top row as if they answered the same question** — that exact mistake caused a real, confusing back-and-forth this session), `cli prop optimize dp` (solve the joint eval+funded value-iteration DP for one plan — a state-dependent risk *function*, not a fixed ladder; see the performance caveat below before reaching for this on a consistency-tracking plan), `cli prop live` (live-account variants).
...
8. **Funded-stage goal: maximize lifetime expected cash extraction, full stop.** Survival is not a second, separately-weighted goal: it's already priced *inside* that one number by the renewal recursion (`lifetimeExpectedNet`, `core/LifetimeExtraction.ts`), where a bust just ends the current cycle and costs one more eval attempt, then a fresh cycle starts. A riskier policy that busts more often only loses if those extra busts cost more in forgone expected cash than they buy in extraction speed; that trade-off is computed, never assumed or weighted as its own thing.
...
**DP performance reality (investigated 2026-09-22 across two optimization passes — read before running `cli prop optimize dp`):** the joint eval+funded DP is mathematically real and already exposed as a permanent CLI command, but it is only *practically fast* for a plan that tracks **no consistency rule on either side**. Confirmed directly: FundedNext Rapid Daily (`consistency eval none | funded none`) solved in ~100 seconds at `--eval-days 20 --iterations 5`. Any plan that tracks an eval OR funded consistency rule — which is most of the registry, including virtually every FTMO Futures/Lucid/Tradeify/TopStep plan — remains impractically slow (both a real, verified ~30-45% cumulative speedup applied across two rounds and the unoptimized original both ran 25-50+ minutes without converging at MFFU Rapid EOD / Tradeify Growth, at identical settings). Both rounds' changes are confirmed bit-for-bit correctness-preserving against both the unoptimized version and each other, and against the existing test suites. **This is a real, current, unresolved limitation, not a stale caveat** — it was re-confirmed, not just repeated, after applying every safe fix found.

The cause, confirmed via profiling both times independently: consistency tracking adds a whole extra bucketed "best day profit" dimension (`bestDay` in `EvalStateValue.ts`, `cycleBestDay` in `FundedStateValue.ts`) to every memoized state. For `FundedStateValue.ts` specifically, tracking funded consistency doesn't just add that one dimension's own bucket count as a multiplier — it forces a full day-tree re-solve once per possible day-start cushion bucket too (since a day's `todayPnL`, hence its best-day-profit contribution, depends on where the day started), so the real multiplier is `bestDayBuckets × cushionBuckets`, not just `bestDayBuckets`.

**What was fixed (safe, applied, verified):** redundant recomputation of the fixed per-cushion candidate-risk grid (was rebuilt from scratch on every call despite depending only on its inputs — now cached/precomputed), string-keyed `Map`s replaced with numeric mixed-radix keys (verified collision-free by construction, not just plausibility), and the "stop trading for the day" leaf value being redundantly recomputed on every trade-index round of the same day-solve despite being identical across all of them except the true idle-day case (now computed once per day-solve and reused).

**What was investigated and deliberately not attempted this session:** real multi-core parallelism via worker threads. This was traced and *proven safe in principle* for `FundedStateValue.ts` specifically — within one sweep level, the per-`(idleDays, cycleBestDay)` solves only read the shared value table and only write to it afterward (a Jacobi update, not Gauss-Seidel), and every cross-level reference is to a level already fully solved by the strictly sequential outer regime/offset loops. The blocker isn't a technical wall, it's scope: the shared value-table snapshot would need to be correctly re-broadcast to workers on every single sweep (not once per level), which means extracting nearly the entire closure-based solver into a worker-callable form — judged to exceed safe "surgical change" scope for a bit-for-bit-critical financial-sizing file in one session, not a dead end. `EvalStateValue.ts` has no equivalent safe boundary at all (its memoization is sparse/reachability-driven, not a dense per-day sweep, so parallelizing it would require restructuring the algorithm itself, not just distributing existing work).

**Before invoking `cli prop optimize dp` on a plan, run `cli prop plans --firm X --variant Y` first and check its `consistency eval`/`funded` fields** — if either is anything other than "none", the DP will be slow-to-impractical; use `cli prop optimize funded`'s flat/percent/ladder sweep instead. Closing this gap for real would need either the worker-thread rearchitecture above (a genuine, multi-hour, high-stakes-to-verify undertaking) or accepting a real accuracy/speed trade-off by coarsening the consistency-tracking grid (not attempted — changes the DP's actual numeric answers, not just its speed).
...
Read `references/firm-rules.md` for account-policy rules the engine doesn't model (account caps, reset/purchase limits, live-transition triggers) — for a plan's actual trading parameters (drawdown, consistency, DLL, payout structure, fees), use `cli prop plans --firm X --variant Y` directly, per RULE #0. See `references/open-questions.md` for what is still unverified.
...
- **Hold the objective constant across a comparison.** Before comparing candidates (firms, plans, sizing policies), state which objective you're optimizing for (lifetime/renewal-adjusted vs. one bounded run; most money vs. fastest; low-bust vs. high-EV) and use the SAME one for every candidate in that comparison. Switching objectives between rows of the same table, or between messages without saying so, is exactly what caused a real, confusing back-and-forth this session.

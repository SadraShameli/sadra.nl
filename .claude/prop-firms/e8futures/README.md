...
**Last Verified:** 2026-09-23 for the product lineup, list prices, discount codes and platform list (against `e8futures.com` directly); 2026-09-19 for everything sourced from the help center (every `e8markets.com` help domain returned HTTP 403 on 2026-09-23, so it could not be re-checked)
**Last Updated:** 2026-09-23
**Source:** see each plan's own file for plan-specific claims; the Firm-Wide Rules below are drawn from help-center articles on `helpfutures.e8markets.com` that explicitly state a firm-wide or cross-product scope. All help-center sources were fetched by the user from their own browser session (Cloudflare blocked every direct/automated fetch attempt this session) on 2026-09-18 and handed back as a combined JSON dump, then cleaned and split into one file per article. The 2026-09-23 product-lineup, pricing and discount checks come from `e8futures.com` pages and the product config embedded in them, fetched directly (HTTP 200). See SOURCES.md.
...
Re-checked live on 2026-09-23 against `e8futures.com` itself. The product config embedded in the site's pages lists ten products. The only three whose platform list is `TRADOVATE` are `e8signature_v_fu` (E8 Signature), `e8zero_max_fu` (E8 Zero MAX) and `e8zero_start_fu` (E8 Zero Starter), each carrying an `_fu` suffix. E8 One and E8 Pro appear there only as `e8one`, `e8pro_v2`, `e8one_perp` and `e8pro_v2_perp` (plus the free-trial variants `e8trialone` and `e8trialone_perp`), and none of those lists Tradovate. `e8futures.com/e8-one` and `e8futures.com/e8-pro` both return HTTP 404, while `e8futures.com/e8-zero` is a live product page. The site's own discount page names them only as "E8 One (Forex)", "E8 Pro (Forex)", "E8 One (Perpetual)" and "E8 Pro (Perpetual)", and its account configurator offers only "E8 Signature", "E8 Zero MAX" and "E8 Zero Starter". This tree therefore documents all three futures products: Signature in `signature.md`, and Zero MAX and Zero Starter together in `zero.md`, since E8's own Zero page states "The rules are identical" between the two tiers.
...
- **Published pass rate: 17.7%, but over a dated window with no product named.** The footer disclaimer on every `e8futures.com` page (fetched 2026-09-23) states: "The evaluation program's customer pass rate from January 1, 2023, to March 1, 2024, was 17.7%, for those who traded at least once during this period and obtained an E8 Trader Account." The "How it works" page says "Roughly 1 in 5 pass (and we actually publish that number)." Neither statement names Signature or Zero, and the window ends in March 2024. Do not assume either plan's current pass rate is 17.7%.
...
- **Standing discount code "E8" is always available** at checkout, per "Are there currently any discounts?": the article does not restate a specific percentage. The percentage and its conditions come only from `e8futures.com`, re-checked 2026-09-23, and E8's own pages disagree with each other there:
    - The discount page lists "E8", "25% off", for "E8 Signature (Futures)", condition "All purchases", and "REB8", "35% off", for "E8 Zero MAX (Futures), E8 Zero Starter (Futures)", condition "All purchases". It also states "checkout accepts one discount code per order."
    - The account configurator applies 25% to Signature ("$160" to "$120" at $50K) and 35% to Zero MAX ("$588" to "$383" at $100K), but attributes both to code "E8" and says "on first order" ("Save $205 with code E8 on first order").
    - The homepage promo shows "$100K Challenge", "40% off", "$353", "$588"; $588 is the Zero MAX $100K list price.

    So the discount percentage per product is corroborated (25% Signature, 35% Zero), but the code name for Zero (E8 vs REB8), whether it applies to every purchase or only the first, and the 40% homepage promo are live conflicts between E8's own pages. Do not assume a discount applies to a repeat purchase or reset.
...
- **Platform: Tradovate is the only supported trading platform for both Futures products**, connectable to TradingView at no extra cost once the trader has logged into Tradovate directly and accepted its own agreements first. Per "Available Trading platforms for Futures," "How can I change my trading platform?," and the two dedicated Tradovate/TradingView setup articles. Market data for Futures products is sourced directly from CME, per "What broker do I trade with?" The site's embedded product config (2026-09-23) agrees: `settings.platforms` is `["TRADOVATE"]` for all three futures products. The `e8futures.com` homepage, however, shows "Best platforms in the Game" followed by "NinjaTrader" and "Tradovate". The help center and the config outrank a marketing-page banner, so Tradovate-only is kept here. Do not assume NinjaTrader is available without checking at checkout.
...
| Eval Fee               | List: $120 / $160 / $280 / $450 (25%-off figures shown: $90 / $120 / $210 / $338; see discount conflict above) | List, 80% share: Starter $178 / $278 / $558, Max $328 / $588 / $1,088 (100% share not displayed, derived only; 35%-off figures and code conflict, see `zero.md`) |
...
- **E8 One and E8 Pro**: per "All product overviews," these are Forex and Perpetual-Futures (crypto) products, not CME futures. Re-confirmed live on 2026-09-23 against `e8futures.com`'s own product config and discount page (see Overview). E8 One's on-demand payout rules (40% Best Day, net profit > 50% of daily drawdown) and E8 Pro's static-drawdown/no-best-day model are genuinely different products with their own separate articles; do not assume any figure in this tree applies to them.
...
- **`help.e8markets.com`** (the general E8 Markets domain, distinct from `helpfutures.e8markets.com`) was not fetched this pass (on 2026-09-23 it, `helpfutures.e8markets.com` and `e8markets.com` all return HTTP 403 to direct fetches). Its sitemap was requested but not yet supplied; every source used in this tree comes from the futures-specific help center only, which appears to be a complete, self-contained help center for both Futures products (it hosts its own copies of KYC, refund, inactivity, and account-limit articles rather than deferring to the general domain). If a future pass fetches `help.e8markets.com`, diff it against this tree's SOURCES.md before assuming nothing new applies to Futures specifically.
- **A source contradiction, now resolved:** two articles ("Available Trading platforms for Futures," "How can I change my trading platform?") both state E8 Zero Futures is "currently the only available product for this market" / that only one platform is offered "for the E8 Zero Futures product," which reads as though E8 Signature Futures does not exist. Per this tree's source-authority rules (a dedicated, plan-specific article outranks a general article on that plan's own facts), the dedicated E8 Signature Futures rules article and the dedicated Signature payout-caps article, plus explicit name-checks in two other articles, control: E8 Signature Futures is treated as a real, currently-sold product, and the two platform articles' claim is treated as stale or inaccurate boilerplate. See `signature.md`'s Other Confirmed Rules section.
- **Engine/doc mismatches** (this skill does not edit the simulator; reported per its own instructions). The three mismatches this bullet listed on 2026-09-20 are fixed in `E8Futures.ts` as of 2026-09-23, confirmed by reading the code:
    - `buildZeroPlan` now passes a separate lock-free Challenge `drawdown` and a locking `fundedDrawdown`.
    - Signature's `minPayoutRequest` is `dollars(125)`.
    - Signature no longer sets `minDaysAfterPassForPayout`.

    Still open as of 2026-09-23:
    - The engine models only the $50K size of each plan.
    - The engine's eval fees mix pricing bases. Signature's `oneTimeEval` is the $160 list price, but the four Zero variants use the discounted prices ($214/$279/$116/$149, 35% off), so the engine's Zero plans look cheaper relative to Signature than their list prices say. See each plan file.
    - Both plans model the reset fee as equal to the eval fee, which no source states.
    - The engine's own notes text says Signature's `minPayoutRequest` was "Corrected 2026-09-18 to dollars(100)", while the code sets `dollars(125)`. The code matches the $125 gross figure; the note is stale.

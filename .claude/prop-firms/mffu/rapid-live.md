...
| Payout Frequency | Daily, resolved 2026-09-22 (see below) | Daily                          | Daily                          | Daily                          |
...
**Payout Frequency, resolved 2026-09-22.** This file's own dedicated "Understanding Rapid Live" source states Daily directly, in its own Live Account Parameters table. MFF's firm-wide "Understanding Live Funded Account at MyFunded Futures" article separately frames live payout frequency as "determined based on an agreement between you and our Risk Team." Per this tree's sourcing hierarchy, a dedicated, plan-specific article outranks a firm-wide one on that plan's own figures, so Daily is treated as controlling here. The firm-wide language most plausibly describes a different live product this tree does not document (the "Core Plan"/"Scale Plan" names the same generic FAQ links out to, neither of which has a live sitemap article, see README.md's Documentation Scope) or an initial negotiation window, rather than a genuine contradiction of Rapid Live's own stated cadence.

## Not Confirmed By This Source

- **Daily Loss Limit (Intraday Limit), a confirmed live conflict between two of MFF's own firm-wide sources, not resolvable by authority ranking** — no Rapid-Live-specific source states one, but two firm-wide Live-account articles point in opposite directions and are not reconciled by any source read here: one states "Yes, all Live accounts have different limits to both their position size and Daily Loss Limits (DLL)" while linking only Core/Scale/Pro-specific figures, never a Rapid one; the other lists a "Customizable Daily Loss Limit: You can set a daily loss limit tailored to your risk tolerance and trading style." Neither is a dedicated Rapid Live article, both are firm-wide/general, and neither citation carries a date that establishes one as newer than the other, so no methodology rule breaks the tie; this stays a genuine, unresolved conflict rather than a picked value. Do not assume Rapid Live has no daily loss limit, and do not assume any Core/Scale/Pro figure applies to it. Weak, non-affirmative supporting context: MFF's own live pricing page (`myfundedfutures.com/plans/rapid`) has a "LIVE ACCOUNT" data table listing Initial Balance, Max Loss Limit, Drawdown Type, Drawdown Floor, Max Contracts, Payout Frequency, and Profit Split (independently verified via raw HTML/JSON, matching this file's already-confirmed Live Account Parameters table exactly), with no Daily Loss Limit row anywhere in it. Absence from a comprehensive-looking table is suggestive, not a source affirmatively stating "none."

...
| Max Payout per Cycle                 | No stated maximum — no source gives a per-request ceiling anywhere (unlike Pro's explicit $100,000 or Builder's explicit $2,000), consistent with Rapid EOD's identical finding for its own otherwise-shared payout structure; see Not Confirmed. |
...
- **Max Payout per Cycle — reasoned inference, not a direct statement** — no source states a per-request ceiling for Rapid. Rapid EOD's own dedicated source (otherwise sharing Rapid's payout structure) explicitly has "no stated maximum" for the same row, and neither plan's sources mention a per-request cap the way Pro ($100,000) and Builder ($2,000) do explicitly. Treated as no evidence of a cap, matching Rapid's own already-established Lifetime Cap finding — moderate, not high, confidence; this is an absence-of-evidence inference, not a quoted "no maximum" statement.

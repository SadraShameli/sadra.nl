...
| News Trading             | Yes (T1), independently confirmed firm-wide by "News Trading Policy": "Unrestricted Accounts: Trading on T1 News events is permitted for the following: All evaluations" | Yes (T1) | Yes (T1) |
...
| Minimum Balance (ongoing)      | No separate figure — the MLL floor documented above (pre-lock: trailing at the stated distance below the high-water mark; post-lock: the static locked value) is the minimum balance by construction, consistent with every other MFF plan researched (Rapid, Rapid EOD, Builder all confirm an MLL breach is an automatic account closure). Confirmed absent as a *separate published field* by a full scan of `myfundedfutures.com/plans/pro`'s raw HTML — not merely unfound. | Same reasoning | Same reasoning |
| Daily Loss Limit               | None | None | None |
| Max Contracts                  | 5 mini / 5 micro | 10 mini / 10 micro | 15 mini / 15 micro |
| Consistency Rule               | None | None | None |
| News Trading                   | Not Allowed, independently confirmed firm-wide by "News Trading Policy": "Restricted Accounts: Trading on T1 News events is prohibited for the following: Rapid Sim Funded, Pro Sim Funded" | Not Allowed | Not Allowed |
| Inactivity Rule                | Unconfirmed | Unconfirmed | Unconfirmed |
...
**Max Active/Concurrent Accounts, now directly confirmed:** "Moving from Evaluation to Sim-Funded Account" (found via a full sitemap sweep of MFF's help center, not the earlier search-driven pass that originally left this row as an inference) states explicitly: "If a trader holds any $100K or $150K Sim-Funded Account, the maximum number of active Sim-Funded Accounts is 3. The 3-account limit applies across all Sim-Funded account sizes and plan types," with worked examples (e.g. "1 × $100K + 2 × $50K = Allowed"). This firm-wide article also confirms the $25K/$50K-only 5-account bucket is genuinely shared across plan types, not per-plan-family, with three named plan-specific exceptions that override it (Rapid EOD 25K/50K: 3; Builder 25K: 2; Builder 50K: 1); Pro is not among the plan-specific exceptions, so a Pro $50K account draws from the same shared 5-account bucket as every other $25K/$50K plan. See README.md's Firm-Wide Rules for the full account-limit structure.

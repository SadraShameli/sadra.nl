...
- **Reset Fee** — Not stated in the cited source. The source lists one-time eval fees ($153 / $125) but does not mention a reset/re-qualification fee. Do not assume reset cost equals eval cost without explicit confirmation.
...
- https://myfundedfutures.com/plans/builder — fetched live (not pasted) via WebFetch's own summarization pass, not a raw-HTML read; no "last updated" date visible. Source of the third-source corroboration for Max Active/Concurrent Accounts above; also independently restates Profit Target, MLL, DLL, Buffer, Minimum Payout, Inactivity Rule (7 days), and Cooldown (21 days) already confirmed by the raw-HTML sources. Its pricing figures ($75 promo / $125 standard) do not clearly map onto this file's already-confirmed $153 (Default) / $125 (Add-On) one-time eval fee and are deliberately not used here to avoid introducing a false conflict with higher-confidence, raw-HTML-sourced data.

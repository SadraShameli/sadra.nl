...
const REPORTABLE_TRPC_CODES = new Set<TRPCError['code']>([
...
const sentryMiddleware = t.middleware(async ({ ctx, next, path, type }) => {
    ...
    const sentryUser = userId ? { id: userId } : undefined;

    try {
        const result = await next();
        if (!result.ok) {
            const error = result.error;
            if (REPORTABLE_TRPC_CODES.has(error.code)) {
...
const tracedProcedure = t.procedure.use(sentryMiddleware);
...
export const adminProcedure = protectedProcedure.use(({ ctx, next }) => {
    const effective = resolveRole(
        ctx.session.user.email,
        ctx.session.user.role,
    );
    if (!isAdminOrAbove(effective)) {
...
export const deviceProcedure = tracedProcedure.use(async ({ ctx, next }) => {
    const header =

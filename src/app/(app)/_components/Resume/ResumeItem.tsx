import parse from 'html-react-parser';
import {
    CalendarDays,
    ChevronRight,
    Cog,
    ExternalLink,
    MapPin,
} from 'lucide-react';
import Image from 'next/image';
import Link from 'next/link';

import type { ResumeSectionView } from '~/lib/content';
import { cn } from '~/lib/utils';

import RevealAnimation from '~/components/ui/Animations/Reveal';
import { Card } from '~/components/ui/Card';

type ResumeProps = {
    title: string;
    sections: ResumeSectionView[];
};

export default function ResumeItem({ title, sections }: ResumeProps) {
    return (
        <RevealAnimation className="pt-spacing-inner">
            <Card className="container">
                <div className="mt-sp grid max-w-3xl gap-y-10 py-spacing-inner lg:py-0">
                    {sections.map((section, index) => {
                        return (
                            <div
                                key={`${section.title}-${section.date}-${index}`}
                            >
                                {section.imageUrl ? (
                                    <div
                                        className={cn(
                                            index && 'border-t pt-12',
                                        )}
                                    >
                                        <Image
                                            className="mb-12 rounded-2xl"
                                            src={section.imageUrl}
                                            alt={`${title} picture`}
                                            width={800}
                                            height={600}
                                            unoptimized
                                        />
                                    </div>
                                ) : null}

                                <div
                                    className={cn(
                                        'flex flex-col justify-between lg:flex-row lg:items-center',
                                        index &&
                                            !section.imageUrl &&
                                            'border-t pt-12',
                                    )}
                                >
                                    {section.url ? (
                                        <Link
                                            className="flex w-fit items-center border-b border-dashed border-transparent text-2xl leading-none font-semibold text-white transition hover:border-current"
                                            href={section.url}
                                        >
                                            <ExternalLink />
                                            <span className="pl-2">
                                                {section.title}
                                            </span>
                                        </Link>
                                    ) : (
                                        <span className="flex w-fit items-center text-2xl leading-none font-semibold text-white">
                                            {section.title}
                                        </span>
                                    )}
                                    {section.role ? (
                                        <span className="mt-1 bg-gradient-neutral-anim text-lg font-semibold lg:mt-auto">
                                            {section.role}
                                        </span>
                                    ) : null}
                                </div>

                                <div className="mt-5 grid gap-y-2 text-neutral-400">
                                    {section.location ? (
                                        section.location.url ? (
                                            <Link
                                                className="flex w-fit items-center transition hover:text-white"
                                                href={section.location.url}
                                            >
                                                <MapPin className="size-5" />
                                                <span className="ml-1">
                                                    {section.location.title}
                                                </span>
                                            </Link>
                                        ) : (
                                            <span className="flex w-fit items-center">
                                                <MapPin className="size-5" />
                                                <span className="ml-1">
                                                    {section.location.title}
                                                </span>
                                            </span>
                                        )
                                    ) : null}

                                    <div className="flex items-center">
                                        <CalendarDays className="size-5" />
                                        <span className="ml-1 leading-none">
                                            {section.date}
                                        </span>
                                    </div>
                                </div>

                                {section.summary ? (
                                    <p className="mt-7 border-t pt-7 text-justify">
                                        {parse(section.summary)}
                                    </p>
                                ) : null}

                                {section.highlights?.length ? (
                                    <ul className="mt-2 grid gap-y-3">
                                        {section.highlights.map(
                                            (highlight, hi) => {
                                                return (
                                                    <li
                                                        className="flex text-justify"
                                                        key={`${hi}-${highlight}`}
                                                    >
                                                        <div className="mt-0.5 mr-1">
                                                            <ChevronRight className="size-5" />
                                                        </div>
                                                        {highlight}
                                                    </li>
                                                );
                                            },
                                        )}
                                    </ul>
                                ) : null}

                                {section.skills?.length ? (
                                    <div className="mt-5 flex items-center font-semibold">
                                        <Cog className="size-5 shrink-0" />
                                        <ul className="ml-2 flex flex-wrap gap-x-1">
                                            {section.skills.map((skill, si) => {
                                                return (
                                                    <li
                                                        className="after:content-[','] last:after:hidden"
                                                        key={`${si}-${skill}`}
                                                    >
                                                        {skill}
                                                    </li>
                                                );
                                            })}
                                        </ul>
                                    </div>
                                ) : null}
                            </div>
                        );
                    })}
                </div>
            </Card>
        </RevealAnimation>
    );
}

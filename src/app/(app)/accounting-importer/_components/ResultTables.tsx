...
import type {
    ...
    MatchAudit,
    UnknownMerchant,
} from '~/lib/accounting-importer/core/types';
...
import {
    VAT_CODES,
    type VatCode,
} from '~/lib/accounting-importer/providers/eboekhouden/enums';
            ...
            },
            {
                cell: ({ row }) => (
                    <Button
                        onClick={async () => {
                            const snippet = `{
    display: '${row.original.rawName}',
    ledger: LEDGERS.SOFTWARE,
    match: '${row.original.rawName.toLowerCase().replaceAll(' ', '')}',
    vatCode: '${row.original.direction === 'IN' ? 'BU_EU_VERK' : 'BI_EU_INK'}',
},`;
                            try {
                                await navigator.clipboard.writeText(snippet);
                                toast.success('Rule snippet copied');
                            } catch {

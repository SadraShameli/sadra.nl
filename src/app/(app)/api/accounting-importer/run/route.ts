...
import {
    ...
    runPlan,
} from '~/lib/accounting-importer/runner';
...
import { runPlanRequestSchema } from '~/lib/schemas/accounting-importer';
...
export async function POST(request: NextRequest) {
    ...
    const ok = await checkRateLimit({
        bucket: 'accounting-importer:run',
        key: userId,
        max: 20,
        windowMs: 60 * 60 * 1000,
    });
    ...
    for (const credentialId of input.apiCredentialIds) {
        try {
            ...
            captureError(error, {
                fields: { credentialId },
                tag: 'accounting-importer.run.loadCredential',
            });
        }
    }

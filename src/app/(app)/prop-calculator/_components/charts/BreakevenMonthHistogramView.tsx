...
const chartConfig: ChartConfig = {
    count: { color: 'hsl(142 76% 45%)', label: 'Trials' },
};

const BIN_COUNT = 20;

export default function BreakevenMonthHistogramView({ result }: Properties) {
    if (result.breakEvenMonthValues.length === 0) {
        return (
            <div className="flex h-64 items-center justify-center text-sm text-muted-foreground">
                No trials went cash-flow positive within this horizon.
            </div>
        );
    }

    const bins = histogram(result.breakEvenMonthValues, BIN_COUNT).map((b) => ({
        center: b.binCenter,
        count: b.count,
    }));
    ...
    return (
        <ChartContainer
            className={cn(
                'app-prop-calculator__breakeven-month-histogram',

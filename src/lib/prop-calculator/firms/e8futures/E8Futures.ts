...
export class E8Futures extends TradingFirm {
    ...
    readonly notes = [
        ...
        "E8 Zero (MAX and Starter) is a real, separate, currently-purchasable one-phase product line, checked live 2026-09-15 directly against e8futures.com/e8-zero's own account configurator (cross-checked against a user-supplied screenshot of the same live configurator showing both the 80% and 100% payout states, which resolved an initial mis-read of the page's default-loaded state). At $50K: Challenge profit target $3,000, 3% EOD Dynamic Drawdown ($1,500), 40% consistency rule (challenge-only -- zero consistency, zero daily loss limit, zero minimum-profitable-days once funded, unlike Signature which is the reverse). Modeled as 4 separate variants (ZeroMax80/100, ZeroStarter80/100) since the payout share is a real, distinct-price choice made at purchase, not a runtime toggle: MAX $214/$279 (80%/100% payout), Starter $116/$149 (80%/100%). Both tiers share identical rules and drawdown; only price, payout share and the per-cycle payout cap ($3,000 MAX vs $1,000 Starter, at $50K) differ. Pending re-confirmation: this repo's own re-audited zero.md doc tree (2026-09-20) found no citable source for these four dollar figures in its own bundle and explicitly declined to treat the configurator/screenshot check above as a cited source -- do not treat $214/$279/$116/$149 as settled until a fresh configurator capture is added to zero.md/SOURCES.md.",
